<template>
  <div class="call">
    ...
    <div class="conversation" v-if='status.conversation' :class="{'fullscreen':!fullscreenStyle}" ref="conversation">
      ...
      <video autoplay='autoplay' ref='localStream' muted="" id="localStream" :class="[videoshow ? 'small' : 'big']"></video>
      <video autoplay ref='remoteStream' id="remoteStream" :class="[videoshow ? 'big' : 'small']"></video>
      <div class="show">
...
export default {
  ...
  mounted() {
    BrowserWindow.setSize(800, 600, true);
  ...
  computed: {
    ...
    userItem: function() {
      return Object.keys(this.userList)
        ...
        // .sort((a, b) => parseInt(a.nickname, 36) - parseInt(b.nickname, 36));
...
.call {
  ...
  .list {
    ...
    >.right {
      ...
      background-size: cover; // max-height: 500px; // padding-top: 80px;
      position: relative;
      .self-url {
        width: 60px;
        height: 60px;
