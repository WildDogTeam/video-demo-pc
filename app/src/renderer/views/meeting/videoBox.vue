<template>
  <div class="video-box">
    <ul class="inner clearfix">
      <li class="item">
        <video muted autoplay="autoplay" ref="localStream"></video>
        <div class="controls">
      ...
      <li class="item" v-for='(item, index) in participants' :key="item.id">
        <video autoplay="autoplay" ref="remoteStream"></video>
        <div class="controls">
          <span class="name" ref="name">{{(item.attributes && item.attributes.name) || 'May'}}</span>
...
export default {
  ...
  methods: {
    _addStream(stream) {
      console.log(stream);
      if (stream.attributes && stream.attributes.isExternalInput == true) {
